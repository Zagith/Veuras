...
public class PageComponent {
    public Vector2 PagePosition {get; set;}
...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler {
    ...
    private List<PageComponent> _pagePositions = new List<PageComponent>();
    ...
    private void SetPagePositions() {
        ...
        for (int i = 0; i < _pageCount; i++) {
            ...
            _pagePositions.Add(new PageComponent{
                PagePosition = -childPosition,
                Name = child.name,
                PageNumber = i
            });
        }
    ...
    private void SetPage(int aPageIndex) {
        ...
        _container.anchoredPosition = _pagePositions[aPageIndex].PagePosition;
    ...
    public void LerpToPage(int aPageIndex) {
        aPageIndex = Mathf.Clamp(aPageIndex, 0, _pageCount - 1);
        _lerpTo = _pagePositions[aPageIndex].PagePosition;
        _lerp = true;
        _currentPage = aPageIndex;
    }
    ...
    private int GetNearestPage() {
        ...
        for (int i = 0; i < _pagePositions.Count; i++) {
            float testDist = Vector2.SqrMagnitude(currentPosition - _pagePositions[i].PagePosition);
